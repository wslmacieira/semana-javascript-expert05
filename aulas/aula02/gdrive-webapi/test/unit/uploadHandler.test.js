...
describe('#UploadHandler test suite', () => {
  ...
  describe('#registerEvents', () => {
    test('should call onFile and onFinish functions on Busboy instance', () => {
      ...
      const busboyInstance = uplodaHandler.registerEvents(headers, onFinish)
  ...
  describe('#handleFileBytes', () => {
    ...
    test('given message timerDelay as 2secs it should emit only two message during 2 seconds period', async () => {
      ...
      expect(firstCallResult).toEqual([handler.ON_UPLOAD_EVENT, { proccessedAlready: "hello".length, filename }])
      expect(secondCallResult).toEqual([handler.ON_UPLOAD_EVENT, { proccessedAlready: messages.join("").length, filename }])
    })
  })
